...
export async function marketplaceExchangeSsoToken(client, request, options) {
    ...
    const [result] = await M.match(M.json(200, ExchangeSsoTokenResponseBody$inboundSchema), M.jsonErr(400, VercelBadRequestError$inboundSchema), M.jsonErr(404, VercelNotFoundError$inboundSchema), M.fail([500, "5XX"]), M.fail("4XX"))(response, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
