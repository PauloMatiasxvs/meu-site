...
import { GetDeploymentFileContentsRequest$outboundSchema, } from "../models/getdeploymentfilecontentsop.js";
 ...
 */
export async function deploymentsGetDeploymentFileContents(client, request, options) {
    const parsed = safeParse(request, (value) => GetDeploymentFileContentsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.jsonErr(400, VercelBadRequestError$inboundSchema), M.jsonErr(401, VercelForbiddenError$inboundSchema), M.fail([403, 410, "4XX"]), M.jsonErr(404, VercelNotFoundError$inboundSchema), M.nil("2XX", z.void()), M.fail("5XX"))(response, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
}
//# sourceMappingURL=deploymentsGetDeploymentFileContents.js.map