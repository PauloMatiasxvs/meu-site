...
import { ArtifactExistsRequest$outboundSchema, } from "../models/artifactexistsop.js";
 ...
 */
export async function artifactsArtifactExists(client, request, options) {
    const parsed = safeParse(request, (value) => ArtifactExistsRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.nil(200, z.void()), M.jsonErr(400, VercelBadRequestError$inboundSchema), M.jsonErr(401, VercelForbiddenError$inboundSchema), M.fail([402, 403, "4XX"]), M.jsonErr(404, VercelNotFoundError$inboundSchema), M.fail("5XX"))(response, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
